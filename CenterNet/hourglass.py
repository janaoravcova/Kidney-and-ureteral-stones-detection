...
from utils import count_parameters, count_flops, load_model
...
class exkp(nn.Module):
    def __init__(self, n, nstack, dims, modules, cnv_dim=256, num_classes=1):
        ...
        self.pre = nn.Sequential(convolution(7, 1, 256, stride=2),
                                 residual(1, 256, curr_dim, stride=2))

        self.kps = nn.ModuleList([kp_module(n, dims, modules) for _ in range(nstack)])

        self.cnvs = nn.ModuleList([convolution(1, curr_dim, cnv_dim) for _ in range(nstack)])

        self.inters = nn.ModuleList([residual(1, curr_dim, curr_dim) for _ in range(nstack - 1)])
    ...
    def forward(self, image):
        ...
        print("Dimension before network {}".format(image.shape))
        inter = self.pre(image)
        print("Dimension after network {}".format(image.shape))
        ...
        print("HG output dimension {}".format(outs[0][1].shape))
        return outs
...
if __name__ == '__main__':
    ...
    print('initializing random values')
    with torch.no_grad():
